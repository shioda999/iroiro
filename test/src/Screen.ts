...
export class Screen {
    ...
    private setPosition = () => {
        ...
        this.container.x = (document.documentElement.clientWidth - Global.WIDTH) / 2
        this.container.y = (document.documentElement.clientHeight - Global.HEIGHT) / 2

        const frame = new PIXI.Graphics()
        frame.beginFill(0,0)
        frame.lineStyle(Math.max(this.container.x, this.container.y), 0x1099bb,1.0, 1)
        frame.drawRect(this.container.x, this.container.y, Global.WIDTH, Global.HEIGHT)
        frame.endFill()
        frame.zIndex = 1
