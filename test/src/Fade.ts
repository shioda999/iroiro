...
export class Fade{
    ...
    constructor(container: PIXI.Container, private callback: () => any){
        ...
        effect.filterArea = new PIXI.Rectangle(container.x, container.y, Global.WIDTH, Global.HEIGHT);
        container.addChild(effect);
        try{
            this.filter = new PIXI.Filter(null, shaderFrag,
                {time: 0,
                len: 800,
                center_x : container.x + Global.WIDTH / 2,
                center_y : container.y + Global.HEIGHT / 2
                });
